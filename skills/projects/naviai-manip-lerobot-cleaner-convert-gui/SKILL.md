...
- 新增 `CHANGELOG.md` 记录必须放在带版本号的标题下，格式为 `## [vMAJOR.MINOR.PATCH] - YYYY-MM-DD`，允许追加预发布后缀；不得继续把新记录直接写入无版本号的 `Unreleased`。版本号必须来自仓库 tag、包发布配置或用户确认；来源不一致或无法确认时先停止并确认，不自行编造。历史无版本号标题保持不变。
